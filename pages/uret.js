...
export default function Uret() {
  ...
  var [adim, setAdim] = useState(1); // 1: form, 2: sonuç
  ...
  async function uret() {
    ...
    try {
      ...
      if (data.senaryo) {
        ...
        alert("Senaryo üretilemedi. Tekrar dene.");
      }
    } catch (e) {
  ...
  async function kaydet() {
    ...
    if (!r.error) {
      ...
      alert("Kaydedilemedi: " + r.error.message);
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif", opacity: loaded ? 1 : 0, transition: "opacity 0.3s ease", paddingBottom: 100 }}>
        ...
        {adim === 1 && (
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: C.muted, marginBottom: 10, letterSpacing: "0.05em", textTransform: "uppercase" }}>Format</p>
            ...
            {/* Tür seçimi */}
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: C.muted, marginBottom: 10, letterSpacing: "0.05em", textTransform: "uppercase" }}>Tür</p>
            ...
            <div style={{ marginBottom: 24 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: C.muted, marginBottom: 10, letterSpacing: "0.05em", textTransform: "uppercase" }}>Özel İstek <span style={{ fontWeight: 400, textTransform: "none" }}>(isteğe bağlı)</span></p>
              <textarea value={ozelIstek} onChange={function (e) { setOzelIstek(e.target.value); }} placeholder="Örn: İstanbul'da geçsin, ana karakter kadın olsun, sürpriz bir final olsun..." rows={3} style={{ width: "100%", background: C.surface, border: "1.5px solid " + C.border, borderRadius: 14, padding: "12px 16px", color: C.text, fontSize: 14, outline: "none", resize: "none", fontFamily: "inherit", lineHeight: 1.6, transition: "border-color 0.2s" }}
            ...
            <button onClick={uret} disabled={yukleniyor} style={{ width: "100%", padding: "16px", borderRadius: 16, background: yukleniyor ? C.input : `linear-gradient(135deg, ${ACCENT}, #c5180a)`, border: "none", color: yukleniyor ? C.muted : "#fff", fontSize: 16, fontWeight: 800, letterSpacing: "-0.01em", boxShadow: yukleniyor ? "none" : `0 6px 24px ${ACCENT}40`, transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
              {yukleniyor ? (
                <>
                  <div style={{ width: 20, height: 20, border: "2.5px solid rgba(255,255,255,0.3)", borderTopColor: TEAL, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
            ...
            {!user && (
              <p style={{ textAlign: "center", fontSize: 12, color: C.muted, marginTop: 12 }}>
                Giriş yapmadan da üretebilirsin — ama kaydetmek için{" "}
                <a href="/" style={{ color: TEAL, fontWeight: 600 }}>giriş yap</a>
              </p>
            )}
          </div>
        )}
        ...
        {adim === 2 && senaryo && (
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
              <button onClick={function () { setAdim(1); setSenaryo(null); setKaydedildi(false); }} style={{ flex: 1, padding: "11px", borderRadius: 12, background: C.surface, border: "1px solid " + C.border, color: C.muted, fontSize: 14, fontWeight: 600 }}>← Geri</button>
              <button onClick={uret} disabled={yukleniyor} style={{ flex: 2, padding: "11px", borderRadius: 12, background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, border: "none", color: "#fff", fontSize: 14, fontWeight: 700, boxShadow: `0 4px 16px ${TEAL}35` }}>
                {yukleniyor ? "Üretiliyor..." : "🔄 Yeniden Üret"}
              </button>
            </div>

            {/* Senaryo kartı */}
            <div style={{ background: C.metalGrad, border: "1px solid " + C.border, borderRadius: 20, overflow: "hidden", boxShadow: C.shadow, marginBottom: 16 }}>
            ...
            {!kaydedildi ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: C.surface, border: "1px solid " + C.border, borderRadius: 12, cursor: "pointer" }}>
                  <input type="checkbox" checked={paylasimAcik} onChange={function (e) { setPaylasimAcik(e.target.checked); }} style={{ width: 16, height: 16, accentColor: TEAL }} />
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 600, color: C.text }}>Topluluğa paylaş</p>
                    <p style={{ fontSize: 12, color: C.muted }}>Senaryonu topluluk sayfasında görünür yap</p>
                  </div>
                </label>
              ...
              <div style={{ textAlign: "center", padding: "24px", background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 16, animation: "fadeUp 0.3s ease" }}>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                  ...
                  <button onClick={function () { setAdim(1); setSenaryo(null); setKaydedildi(false); }} style={{ background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, border: "none", borderRadius: 10, padding: "8px 18px", color: "#fff", fontSize: 13, fontWeight: 700 }}>Yeni Üret</button>
                </div>
              </div>
            )}
          </div>
      ...
      {/* MOBİL ALT NAV */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: dk ? "rgba(8,15,28,0.95)" : "rgba(255,255,255,0.95)", backdropFilter: "blur(24px)", borderTop: "1px solid " + C.border, padding: "8px 0 env(safe-area-inset-bottom,8px)", display: "flex", justifyContent: "space-around", alignItems: "center", boxShadow: "0 -4px 30px rgba(0,0,0,0.1)" }}>
