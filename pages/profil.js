...
export default function Profil() {
  ...
  async function yukle(u) {
    ...
    if (p) {
      ...
      // Username yoksa hemen düzenleme ekranı aç
      if (!p.username) setTimeout(() => setDuzenle(true), 500);
    }
  ...
  async function profilKaydet() {
    ...
    if (!u) { alert("Kullanıcı adı boş olamaz!"); return; }
    if (u.length < 3) { alert("Kullanıcı adı en az 3 karakter olmalı!"); return; }
    setKaydetYukleniyor(true);
    // Unique kontrol
    var { data: mevcut } = await supabase.from("profiles").select("id").eq("username", u).neq("id", user.id).single();
    if (mevcut) { alert("@" + u + " kullanıcı adı zaten alınmış!"); setKaydetYukleniyor(false); return; }
    await supabase.from("profiles").update({ bio: bioEdit, username: u, website: websiteEdit, gizli: gizliEdit }).eq("id", user.id);
  ...
  async function cloudinarySil(url) {
    ...
    try {
      ...
      var public_id = match ? match[1] : null;
      await fetch("/api/medya-sil", {
        method: "POST",
  ...
  async function avatarSil() {
    ...
    if (!error) setProfil(p => ({ ...p, avatar_url: null }));
    setAvatarYukleniyor(false);
  }

  async function bannerSil() {
    if (!profil?.banner_url) return;
    if (!confirm("Banner silinsin mi?")) return;
    setBannerYukleniyor(true);
    await cloudinarySil(profil.banner_url);
    var { error } = await supabase.from("profiles").update({ banner_url: null }).eq("id", user.id);
    if (!error) setProfil(p => ({ ...p, banner_url: null }));
    setBannerYukleniyor(false);
  }
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,BlinkMacSystemFont,sans-serif", paddingBottom: 80 }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: dk ? "rgba(8,15,28,0.95)" : "rgba(238,242,247,0.95)", backdropFilter: "blur(20px)", borderBottom: "1px solid " + C.border, padding: "10px 16px", display: "flex", alignItems: "center", gap: 10 }}>
        <a href="/" style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg," + TEAL + "," + TEAL_L + ")", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>
          {profil?.avatar_url ? <img src={profil.avatar_url} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" /> : <span style={{ color: "#fff" }}>←</span>}
        </a>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 16, fontWeight: 800, color: C.text, lineHeight: 1 }}>
            {profil?.username ? "@" + profil.username : "Profilim"}
          </p>
          <p style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>{user?.email}</p>
        </div>
        <button onClick={() => setDuzenle(true)} style={{ padding: "7px 14px", borderRadius: 20, border: "1.5px solid " + C.border, background: "none", color: C.text, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Düzenle</button>
        <button onClick={() => { if(confirm("Çıkış yapılsın mı?")) supabase.auth.signOut().then(() => window.location.href = "/"); }} style={{ padding: "7px 14px", borderRadius: 20, border: "1.5px solid " + ACCENT + "40", background: ACCENT + "10", color: ACCENT, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Çıkış</button>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <div style={{ height: 130, background: profil?.banner_url ? "transparent" : "linear-gradient(135deg," + TEAL + "25," + ACCENT + "12)", overflow: "hidden", position: "relative", cursor: "pointer" }} onClick={() => bannerRef.current?.click()}>
          ...
          {profil?.banner_url && (
            <button onClick={e => { e.stopPropagation(); bannerSil(); }} style={{ position: "absolute", top: 8, right: 8, width: 28, height: 28, borderRadius: "50%", background: "rgba(0,0,0,0.6)", border: "none", color: "#fff", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
          )}
        ...
        <div style={{ padding: "0 20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 14, marginTop: -45, marginBottom: 14 }}>
            ...
            {profil?.avatar_url && (
              <button onClick={e => { e.stopPropagation(); avatarSil(); }} style={{ position: "absolute", top: -4, left: -4, width: 22, height: 22, borderRadius: "50%", background: ACCENT, border: "2px solid " + C.bg, color: "#fff", fontSize: 10, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", zIndex: 2 }}>✕</button>
            )}
      ...
      {duzenle && (
        <div>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 401, background: dk ? "#0f1829" : "#fff", borderRadius: "24px 24px 0 0", padding: "10px 20px env(safe-area-inset-bottom,24px)", maxHeight: "85vh", overflowY: "auto" }}>
            ...
            {[{ label: "Kullanıcı Adı", val: usernameEdit, set: (v) => setUsernameEdit(v.toLowerCase().replace(/[^a-z0-9_]/g, "")), prefix: "@", hint: "Sadece harf, rakam ve _ kullanabilirsin" }, { label: "Website", val: websiteEdit, set: setWebsiteEdit, prefix: "🔗", hint: "" }].map(f => (
              <div key={f.label} style={{ marginBottom: 14 }}>
                ...
                {f.hint ? <p style={{ fontSize: 11, color: C.muted, marginTop: 4, marginLeft: 4 }}>{f.hint}</p> : null}
              </div>
            ))}
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: dk ? "rgba(8,15,28,0.97)" : "rgba(255,255,255,0.97)", backdropFilter: "blur(28px)", borderTop: "1px solid " + C.border, padding: "8px 0 env(safe-area-inset-bottom,8px)", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
        {[
          { href: "/", svg: <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
          { href: "/kesfet", svg: <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg> },
          { href: "/topluluk", svg: <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg> },
          { href: "/mesajlar", svg: <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg> },
        ].map(item => {
          var isActive = item.href === "/profil" || (item.href !== "/profil" && false);
          return (
            <a key={item.href} href={item.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "6px 20px", borderRadius: 14, color: C.muted, opacity: 0.5 }}>
              {item.svg}
            </a>
          );
        })}
        {/* Aktif: profil ikonu */}
        <a href="/profil" style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "6px 20px", borderRadius: 14, color: TEAL, position: "relative" }}>
          <svg width="24" height="24" fill="none" stroke={TEAL} strokeWidth="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          <div style={{ position: "absolute", bottom: 2, width: 18, height: 3, borderRadius: 2, background: TEAL }} />
        </a>
      </div>
    </div>
  );
