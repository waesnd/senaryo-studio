...
export default function Profil() {
  ...
  function temaToggle() { var t = dk ? "light" : "dark"; setTema(t); localStorage.setItem("sf_tema", t); }

  function zaman(ts) {
    var d = Math.floor((Date.now() - new Date(ts)) / 1000);
    if (d < 60) return d + "s önce"; if (d < 3600) return Math.floor(d / 60) + "dk önce";
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif", opacity: loaded ? 1 : 0, transition: "opacity 0.3s ease", paddingBottom: 90 }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .konu-kart{transition:all 0.2s;} .konu-kart:hover{box-shadow:0 4px 24px rgba(8,145,178,0.12) !important;}
      `}</style>

      {/* TOPBAR */}
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: dk ? "rgba(8,15,28,0.92)" : "rgba(238,242,247,0.92)", backdropFilter: "blur(20px)", borderBottom: "1px solid " + C.border, padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={function () { setDuzenle(!duzenle); }} style={{ background: duzenle ? ACCENT : C.input, border: "1px solid " + (duzenle ? ACCENT : C.border), borderRadius: 10, padding: "7px 16px", color: duzenle ? "#fff" : C.muted, fontSize: 13, fontWeight: 700, transition: "all 0.2s" }}>{duzenle ? "İptal" : "Düzenle"}</button>
        </div>
      </div>

      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        {/* Banner */}
        <div style={{ position: "relative", height: 180, background: profil && profil.banner_url ? "transparent" : `linear-gradient(135deg, ${TEAL}60, ${ACCENT}30)`, overflow: "hidden", cursor: "pointer" }} onClick={function () { bannerRef.current && bannerRef.current.click(); }}>
          {profil && profil.banner_url && <img src={profil.banner_url} style={{ width: "100%", height: "100%", objectFit: "cover" }} />}
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {bannerYuk ? <div style={{ width: 28, height: 28, border: "3px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> : <div style={{ background: "rgba(0,0,0,0.35)", borderRadius: 10, padding: "7px 14px", color: "#fff", fontSize: 12, fontWeight: 600, backdropFilter: "blur(8px)" }}>📷 Kapak Değiştir</div>}
          </div>
        ...
        <div style={{ padding: "0 20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: -46, marginBottom: 14 }}>
            <div style={{ position: "relative", cursor: "pointer" }} onClick={function () { avatarRef.current && avatarRef.current.click(); }}>
              <div style={{ width: 90, height: 90, borderRadius: "50%", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, border: `4px solid ${C.bg}`, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>
                {avatarYuk ? <div style={{ width: 26, height: 26, border: "3px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> : avatarUrl ? <img src={avatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "👤"}
              </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={function () { window.location.href = "/mesajlar"; }} style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 20, padding: "8px 18px", color: C.text, fontSize: 13, fontWeight: 700 }}>💬 Mesaj</button>
          ...
          <div style={{ display: "flex", marginBottom: 18, borderBottom: "1px solid " + C.border }}>
            {[{ id: "gonderiler", label: "Gönderiler" }, { id: "senaryolar", label: "Senaryolar" }].map(function (t) {
              ...
              return <button key={t.id} onClick={function () { setTab(t.id); }} style={{ flex: 1, padding: "12px 8px", background: "none", border: "none", borderBottom: isActive ? `2px solid ${TEAL}` : "2px solid transparent", color: isActive ? TEAL : C.muted, fontSize: 13, fontWeight: isActive ? 700 : 500, transition: "all 0.2s", marginBottom: "-1px", cursor: "pointer" }}>{t.label}</button>;
            })}
          </div>

          {tab === "gonderiler" && (
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              {gonderiler.length === 0 ? <div style={{ textAlign: "center", padding: "60px 0" }}><p style={{ fontSize: 40 }}>📭</p><p style={{ fontSize: 14, color: C.muted, marginTop: 10 }}>Henüz gönderi yok.</p></div>
          ...
          {tab === "senaryolar" && (
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              {konular.length === 0 ? (
                ...
                return (
                  <div key={k.id} className="konu-kart" style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 16, padding: "16px 18px", marginBottom: 10, boxShadow: C.shadow, animation: `fadeUp 0.3s ${i * 0.04}s both ease`, overflow: "hidden", position: "relative" }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                      ...
                      {k.paylasim_acik && <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 20, background: "rgba(16,185,129,0.08)", color: "#10b981" }}>🌍 Paylaşık</span>}
                    </div>
                    <h3 style={{ fontSize: 15, fontWeight: 800, color: C.text, marginBottom: 5 }}>{k.baslik}</h3>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: dk ? "rgba(8,15,28,0.96)" : "rgba(255,255,255,0.96)", backdropFilter: "blur(28px)", borderTop: "1px solid " + C.border, padding: "10px 0 env(safe-area-inset-bottom,10px)", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
        {[{ icon: "🏠", href: "/" }, { icon: "🔭", href: "/kesfet" }, { icon: "🎭", href: "/topluluk" }, { icon: "💬", href: "/mesajlar" }].map(function (item) {
          return (
            <a key={item.href} href={item.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "6px 24px", borderRadius: 14, textDecoration: "none", position: "relative" }}>
              <span style={{ fontSize: 26, lineHeight: 1, filter: "grayscale(50%) opacity(0.45)" }}>{item.icon}</span>
            </a>
          );
        })}
      </div>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 201, width: 300, background: dk ? "#0d1627" : "#fff", boxShadow: "4px 0 40px rgba(0,0,0,0.25)", display: "flex", flexDirection: "column", animation: "slideIn 0.28s cubic-bezier(0.34,1.2,0.64,1)" }}>
            ...
            <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid " + C.border, flexShrink: 0 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, cursor: "pointer", border: `3px solid ${dk ? "#0d1627" : "#fff"}`, outline: `2px solid ${TEAL}30` }}>
                  {avatarUrl ? <img src={avatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "👤"}
                </div>
              ...
              <p style={{ fontSize: 12, color: C.muted }}>{user && user.email}</p>
            </div>
            <nav style={{ flex: 1, overflowY: "auto", padding: "12px 12px" }}>
              {DRAWER_MENU.map(function (item) {
                var isActive = item.href === "/profil";
                return (
              ...
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid " + C.border }}>
                ...
                <a href="/profil" onClick={function () { setDrawer(false); }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 14, color: TEAL, fontWeight: 700, fontSize: 15, marginBottom: 4, background: TEAL + "10", border: `1px solid ${TEAL}25`, textDecoration: "none" }}>
                ...
                <button onClick={function () { supabase.auth.signOut(); setDrawer(false); window.location.href = "/"; }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 14, color: ACCENT, fontWeight: 600, fontSize: 15, background: ACCENT + "08", border: "none", width: "100%", textAlign: "left", cursor: "pointer", marginTop: 4 }}>
