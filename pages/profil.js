...
export default function Profil() {
  ...
  async function cloudinarySil(url) {
    if (!url) return;
    try {
      // URL'den public_id çıkar: .../upload/v123/KLASOR/DOSYA.jpg → KLASOR/DOSYA
      var parca = url.split("/upload/")[1];
      if (!parca) return;
      var public_id = parca.replace(/^v\d+\//, "").replace(/\.[^/.]+$/, "");
      await fetch("/api/medya-sil", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ public_id }) });
    } catch (e) {}
  }
