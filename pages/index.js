...
function getC(dk) {
  return {
    ...
    glass: dk ? "rgba(15,24,41,0.82)" : "rgba(255,255,255,0.82)",
    metalGrad: dk
      ? "linear-gradient(145deg,#1a2740 0%,#0f1829 60%,#162035 100%)"
      : "linear-gradient(145deg,#ffffff 0%,#f0f4f8 60%,#e8eef5 100%)",
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif", opacity: loaded ? 1 : 0, transition: "opacity 0.3s ease" }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .nav-btn:hover{color:${TEAL} !important;}
        .feed-kart{transition:box-shadow 0.2s ease;}
        .feed-kart:hover{box-shadow:0 4px 28px rgba(8,145,178,0.12) !important;}
        ...
        @media(max-width:768px){
          ...
          .feed-container{padding-bottom:80px !important;}
        }
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "240px minmax(0,1fr) 300px", minHeight: "100vh" }}>
        ...
        <aside className="desktop-sidebar" style={{ position: "sticky", top: 0, height: "100vh", borderRight: "1px solid " + C.border, background: C.metalGrad, display: "flex", flexDirection: "column", padding: "20px 14px 20px", overflowY: "auto", backdropFilter: "blur(20px)" }}>
          ...
          <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
            {NAV.map(function (item) {
              ...
              return (
                <a key={item.href} href={item.href} className="nav-btn" style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 13px", borderRadius: 12, color: isActive ? TEAL : C.text, background: isActive ? TEAL + "15" : "transparent", fontWeight: isActive ? 700 : 500, fontSize: 14, border: "1px solid " + (isActive ? TEAL + "30" : "transparent") }}>
                  <span style={{ fontSize: 18, width: 22, textAlign: "center", color: isActive ? TEAL : C.muted }}>{item.icon}</span>
                  {item.label}
                </a>
              );
            })}
            ...
            {/* Senaryo Üret */}
            <a href="/uret" style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 13px", borderRadius: 12, color: ACCENT, background: ACCENT + "10", fontWeight: 700, fontSize: 14, border: "1px solid " + ACCENT + "25", marginTop: 4 }}>
              <span style={{ fontSize: 18, width: 22, textAlign: "center" }}>🎬</span>
              Senaryo Üret
              <span style={{ marginLeft: "auto", fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 20, background: ACCENT, color: "#fff" }}>AI</span>
            </a>

            <button onClick={function () { user ? setGonderiAcik(true) : setAuthModal(true); }} style={{ marginTop: 10, width: "100%", padding: "11px", borderRadius: 12, background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, border: "none", color: "#fff", fontSize: 14, fontWeight: 700, boxShadow: `0 4px 16px ${TEAL}40` }}>
              + Paylaş
            </button>
          </nav>
          ...
          <div style={{ borderTop: "1px solid " + C.border, paddingTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {user ? (
              ...
              <button onClick={function () { setAuthModal(true); }} style={{ padding: "10px 12px", borderRadius: 12, background: `linear-gradient(135deg, ${ACCENT}, #c5180a)`, border: "none", color: "#fff", fontSize: 13, fontWeight: 700, boxShadow: `0 4px 14px ${ACCENT}35` }}>
                Giriş Yap
              </button>
            )}
          </div>
        ...
        <main style={{ borderRight: "1px solid " + C.border, minHeight: "100vh" }}>
          ...
          {/* Header */}
          <div style={{ position: "sticky", top: 0, zIndex: 20, background: dk ? "rgba(8,15,28,0.9)" : "rgba(238,242,247,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid " + C.border, padding: "13px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {/* Mobil logo */}
              <div className="mobile-nav" style={{ alignItems: "center", gap: 8, display: "none" }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: `linear-gradient(135deg, ${ACCENT}, #ff5722)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🎬</div>
                <span style={{ fontSize: 16, fontWeight: 800, color: C.text, letterSpacing: "-0.02em" }}>Scriptify</span>
              </div>
              <h1 className="desktop-sidebar" style={{ fontSize: 16, fontWeight: 800, color: C.text, letterSpacing: "-0.02em" }}>Ana Sayfa</h1>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {user ? (
                <div style={{ width: 34, height: 34, borderRadius: "50%", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, overflow: "hidden", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }} onClick={function () { window.location.href = "/profil"; }}>
                  {avatarUrl ? <img src={avatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "👤"}
                </div>
              ) : (
          ...
          <div className="feed-container" style={{ paddingBottom: 20 }}>
            ...
            <div style={{ padding: "16px 20px 0" }}>
              <a href="/uret" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderRadius: 18, background: dk ? `linear-gradient(135deg, ${TEAL}18, ${ACCENT}10)` : `linear-gradient(135deg, ${TEAL}10, ${ACCENT}07)`, border: `1px solid ${TEAL}30`, marginBottom: 0, transition: "all 0.2s", boxShadow: `0 4px 20px ${TEAL}15` }}
            ...
            {/* Gönderi yaz */}
            <div style={{ padding: "14px 20px", borderBottom: "1px solid " + C.border, background: C.surface, backdropFilter: "blur(10px)", marginTop: 14, borderTop: "1px solid " + C.border }}>
              <div style={{ display: "flex", gap: 12 }}>
                <div style={{ width: 38, height: 38, borderRadius: "50%", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, flexShrink: 0, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, boxShadow: `0 2px 10px ${TEAL}25`, cursor: "pointer" }} onClick={function () { window.location.href = "/profil"; }}>
                  {avatarUrl ? <img src={avatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "👤"}
                </div>
                <div style={{ flex: 1 }}>
            ...
            {gonderiler.length === 0 ? (
              <div style={{ textAlign: "center", padding: "80px 20px", animation: "fadeUp 0.4s ease" }}>
                ...
                <button onClick={function () { user ? setGonderiAcik(true) : setAuthModal(true); }} style={{ background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, border: "none", borderRadius: 12, padding: "10px 24px", color: "#fff", fontSize: 14, fontWeight: 700, boxShadow: `0 4px 16px ${TEAL}35` }}>İlk paylaşımı yap →</button>
              </div>
              ...
              return (
                <div key={g.id} className="feed-kart" style={{ borderBottom: "1px solid " + C.border, padding: "16px 20px", background: C.surface, animation: `fadeUp 0.3s ${Math.min(i * 0.04, 0.25)}s both ease` }}>
                  <div style={{ display: "flex", gap: 12 }}>
                    <div style={{ flexShrink: 0, width: 40, height: 40, borderRadius: "50%", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, boxShadow: `0 2px 10px ${TEAL}20`, cursor: "pointer" }} onClick={function () { window.location.href = "/profil"; }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 2, marginTop: 10 }}>
                        ...
                        <button className="aksiyon-btn" style={{ display: "flex", alignItems: "center", gap: 5, background: "transparent", border: "none", borderRadius: 20, padding: "5px 10px", color: C.muted, fontSize: 12 }}>
                          <span style={{ fontSize: 14 }}>🔁</span>
                        </button>
                        <button className="aksiyon-btn" onClick={function () { begeniToggle(g.id, g.begeni_sayisi || 0); }} style={{ display: "flex", alignItems: "center", gap: 5, background: begendi ? ACCENT + "10" : "transparent", border: "none", borderRadius: 20, padding: "5px 10px", color: begendi ? ACCENT : C.muted, fontSize: 12, fontWeight: begendi ? 700 : 400 }}>
                          <span style={{ fontSize: 14 }}>{begendi ? "❤️" : "♡"}</span><span>{g.begeni_sayisi || ""}</span>
                        </button>
                        <button className="aksiyon-btn" onClick={function () { kaydetToggle(g.id); }} style={{ display: "flex", alignItems: "center", gap: 5, background: kaydetti ? TEAL + "10" : "transparent", border: "none", borderRadius: 20, padding: "5px 10px", color: kaydetti ? TEAL : C.muted, fontSize: 12 }}>
                          <span style={{ fontSize: 14 }}>{kaydetti ? "🔖" : "🏷️"}</span>
                        </button>
                        <button className="aksiyon-btn" onClick={function () { navigator.clipboard.writeText(window.location.origin); }} style={{ display: "flex", alignItems: "center", background: "transparent", border: "none", borderRadius: 20, padding: "5px 10px", color: C.muted, fontSize: 12, marginLeft: "auto" }}>
                          <span style={{ fontSize: 14 }}>↗</span>
                        </button>
                      </div>
                            ...
                            {(yorumlar[g.id] || []).map(function (y) {
                              return (
                                <div key={y.id} style={{ display: "flex", gap: 8 }}>
                                  <div style={{ width: 26, height: 26, borderRadius: "50%", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_L})`, overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11 }}>
        ...
        <aside className="desktop-right" style={{ padding: "22px 18px", position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
          ...
          <div style={{ background: C.metalGrad, border: "1px solid " + C.border, borderRadius: 20, padding: "18px", boxShadow: C.shadow }}>
            ...
            {onerilenler.length === 0 ? <p style={{ fontSize: 12, color: C.muted }}>Henüz kullanıcı yok.</p>
          ...
          <p style={{ fontSize: 10, color: C.muted, textAlign: "center", marginTop: 16, lineHeight: 1.6 }}>© 2025 Scriptify · by Öztürk<br /><span style={{ color: TEAL }}>Sinema severler için ♥</span></p>
        </aside>
      </div>

      {/* ═══ MOBİL ALT NAVIGASYON ═══ */}
      <div className="mobile-nav" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: dk ? "rgba(8,15,28,0.95)" : "rgba(255,255,255,0.95)", backdropFilter: "blur(24px)", borderTop: "1px solid " + C.border, padding: "8px 0 env(safe-area-inset-bottom,8px)", display: "none", justifyContent: "space-around", alignItems: "center", boxShadow: "0 -4px 30px rgba(0,0,0,0.1)" }}>
        {[
          { icon: "⌂", label: "Sayfa", href: "/" },
          { icon: "◎", label: "Keşfet", href: "/kesfet" },
          { icon: "◈", label: "Topluluk", href: "/topluluk" },
          { icon: "◇", label: "Mesajlar", href: "/mesajlar" },
          { icon: "○", label: "Profil", href: "/profil" },
          ...
          var isActive = aktifPath === item.href;
          return (
            <a key={item.href} href={item.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "6px 14px", borderRadius: 12, color: isActive ? TEAL : C.muted, textDecoration: "none", minWidth: 56 }}>
              <span style={{ fontSize: 22, lineHeight: 1 }}>{item.icon}</span>
              <span style={{ fontSize: 9, fontWeight: isActive ? 700 : 500, letterSpacing: "0.02em" }}>{item.label}</span>
              {isActive && <div style={{ width: 18, height: 3, borderRadius: 2, background: TEAL, marginTop: 1 }} />}
            </a>
          );
        })}
      </div>
