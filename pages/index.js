...
export default function Home() {
  ...
  function Avatar({ url, size, fontSize }) {
    return (
      <div style={{ width: size, height: size, borderRadius: "50%", background: "linear-gradient(135deg," + TEAL + "," + TEAL_L + ")", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: fontSize || 14, flexShrink: 0 }}>
  ...
  function DrawerPanel() {
    return (
      <>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 201, width: 300, background: dk ? "#0d1627" : "#fff", boxShadow: "4px 0 40px rgba(0,0,0,0.3)", display: "flex", flexDirection: "column", animation: "slideIn 0.25s ease" }}>
          ...
          <div style={{ padding: "20px", borderBottom: "1px solid " + C.border, flexShrink: 0 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div onClick={function () { setDrawer(false); window.location.href = "/profil"; }} style={{ cursor: "pointer" }}>
                <Avatar url={avatarUrl} size={54} fontSize={22} />
              </div>
              <button onClick={function () { setDrawer(false); }} style={{ background: C.input, border: "1px solid " + C.border, borderRadius: 10, padding: "6px 12px", color: C.muted, fontSize: 13, cursor: "pointer" }}>✕</button>
            </div>
            {user ? (
              <div>
              ...
              <div>
                ...
                <button onClick={function () { setDrawer(false); setAuthModal(true); }} style={{ width: "100%", padding: "10px", borderRadius: 12, background: "linear-gradient(135deg," + ACCENT + ",#c5180a)", border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Giriş Yap</button>
              </div>
            )}
          </div>
          ...
          <nav style={{ flex: 1, overflowY: "auto", padding: "12px" }}>
            {DRAWER_MENU.map(function (item) {
              ...
              return (
                <a key={item.href} href={item.href} onClick={function () { setDrawer(false); }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 14, color: isActive ? TEAL : C.text, background: isActive ? TEAL + "15" : "transparent", fontWeight: isActive ? 700 : 500, fontSize: 15, marginBottom: 4, border: "1px solid " + (isActive ? TEAL + "30" : "transparent"), textDecoration: "none", transition: "all 0.15s" }}>
            ...
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid " + C.border }}>
              ...
              <a href="/profil" onClick={function () { setDrawer(false); }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 14, color: C.text, fontWeight: 500, fontSize: 15, marginBottom: 4, textDecoration: "none" }}>
                <span style={{ fontSize: 22, width: 28, textAlign: "center" }}>👤</span><span>Profil & Ayarlar</span>
              </a>
              <button onClick={function () { temaToggle(); }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 14, color: C.text, fontWeight: 500, fontSize: 15, marginBottom: 4, background: "none", border: "none", width: "100%", textAlign: "left", cursor: "pointer" }}>
                <span style={{ fontSize: 22, width: 28, textAlign: "center" }}>{dk ? "☀️" : "🌙"}</span><span>{dk ? "Açık Tema" : "Koyu Tema"}</span>
              </button>
              {user && (
                <button onClick={function () { supabase.auth.signOut(); setDrawer(false); }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 14, color: ACCENT, fontWeight: 600, fontSize: 15, background: ACCENT + "10", border: "none", width: "100%", textAlign: "left", cursor: "pointer", marginTop: 4 }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif", opacity: loaded ? 1 : 0, transition: "opacity 0.3s ease" }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .fk{transition:box-shadow 0.2s;}
        .fk:hover{box-shadow:0 4px 24px rgba(8,145,178,0.1)!important;}
        @media(max-width:768px){
          .dsb{display:none!important;}
          .drg{display:none!important;}
          .mbn{display:flex!important;}
          .fc{padding-bottom:90px!important;}
        }
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "240px minmax(0,1fr) 300px", minHeight: "100vh" }}>
        ...
        <aside className="dsb" style={{ position: "sticky", top: 0, height: "100vh", borderRight: "1px solid " + C.border, background: C.metalGrad, display: "flex", flexDirection: "column", padding: "20px 14px", overflowY: "auto" }}>
          ...
          <div style={{ borderTop: "1px solid " + C.border, paddingTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
            <button onClick={temaToggle} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 10, background: C.input, border: "1px solid " + C.border, color: C.muted, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
              {dk ? "☀️" : "🌙"} {dk ? "Açık Tema" : "Koyu Tema"}
            </button>
            {user ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 12, background: C.input, border: "1px solid " + C.border, cursor: "pointer" }} onClick={function () { window.location.href = "/profil"; }}>
                <Avatar url={avatarUrl} size={32} fontSize={14} />
        ...
        <main style={{ borderRight: "1px solid " + C.border, minHeight: "100vh" }}>
          ...
          <div className="fc" style={{ paddingBottom: 20 }}>
            ...
            <div style={{ padding: "16px 20px 0" }}>
              <a href="/uret" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderRadius: 18, background: dk ? "linear-gradient(135deg," + TEAL + "18," + ACCENT + "10)" : "linear-gradient(135deg," + TEAL + "10," + ACCENT + "07)", border: "1px solid " + TEAL + "30", boxShadow: "0 4px 20px " + TEAL + "15" }}>
            ...
            <div style={{ padding: "14px 20px", borderBottom: "1px solid " + C.border, background: C.surface, marginTop: 14, borderTop: "1px solid " + C.border }}>
              <div style={{ display: "flex", gap: 12 }}>
                <div style={{ cursor: "pointer" }} onClick={function () { window.location.href = "/profil"; }}>
                  <Avatar url={avatarUrl} size={38} fontSize={15} />
                </div>
                <div style={{ flex: 1 }}>
            ...
            {gonderiler.length === 0 ? (
              ...
              var gAvatarUrl = g.profiles && g.profiles.avatar_url ? g.profiles.avatar_url : null;
              return (
                <div key={g.id} className="fk" style={{ borderBottom: "1px solid " + C.border, padding: "16px 20px", background: C.surface }}>
                  <div style={{ display: "flex", gap: 12 }}>
                    <div style={{ cursor: "pointer" }} onClick={function () { window.location.href = "/profil"; }}>
                      <Avatar url={gAvatarUrl} size={40} fontSize={15} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 2, marginTop: 10 }}>
                        ...
                        <button onClick={function () { navigator.clipboard && navigator.clipboard.writeText(window.location.href); }} style={{ background: "transparent", border: "none", borderRadius: 20, padding: "5px 10px", color: C.muted, fontSize: 14, cursor: "pointer", marginLeft: "auto" }}>↗</button>
                      </div>
                      {yorumAcik && (
                        <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid " + C.border }}>
                          <div style={{ maxHeight: 160, overflowY: "auto", marginBottom: 10, display: "flex", flexDirection: "column", gap: 8 }}>
                            ...
                            {(yorumlar[g.id] || []).map(function (y) {
                              var yAvatarUrl = y.profiles && y.profiles.avatar_url ? y.profiles.avatar_url : null;
                              return (
                                <div key={y.id} style={{ display: "flex", gap: 8 }}>
                                  <Avatar url={yAvatarUrl} size={26} fontSize={11} />
        ...
        <aside className="drg" style={{ padding: "22px 18px", position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
          ...
          <div style={{ background: C.metalGrad, border: "1px solid " + C.border, borderRadius: 20, padding: "18px", marginBottom: 14 }}>
            ...
            {trendler.length === 0 ? <p style={{ fontSize: 12, color: C.muted }}>Henüz yok.</p>
            ...
            {onerilenler.map(function (u, i) {
              return (
                <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: i < onerilenler.length - 1 ? 12 : 0 }}>
                  <Avatar url={u.avatar_url} size={34} fontSize={13} />
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          ...
          <div style={{ position: "relative", zIndex: 1, background: dk ? "#0f1829" : "#fff", border: "1px solid " + C.border, borderRadius: 24, padding: "32px", maxWidth: 380, width: "100%", boxShadow: "0 32px 80px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 11, color: C.muted }}>veya</span>
              <div style={{ flex: 1, height: 1, background: C.border }} />
            </div>
