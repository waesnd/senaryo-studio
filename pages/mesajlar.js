...
function Icon({id,size=22,color="currentColor",strokeWidth=1.8}){
  ...
  if(id==="image")return<svg {...p}><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>;
...
function Av({url,size,online=false}){
  return(
    <div style={{position:"relative",flexShrink:0}}>
      <div style={{width:size,height:size,borderRadius:"50%",background:"linear-gradient(135deg,#1a1500,#2a2000)",border:`1.5px solid rgba(212,175,55,0.2)`,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
        {url?<img src={url} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<Icon id="user" size={size*0.4} color="rgba(212,175,55,0.35)"/>}
      </div>
      {online&&<div style={{position:"absolute",bottom:1,right:1,width:size/4,height:size/4,borderRadius:"50%",background:G.green,border:`2px solid ${G.black}`}}/>}
    </div>
  );
}
...
function AltNav(){
  ...
  return(
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:90,background:"rgba(8,8,8,0.97)",backdropFilter:"blur(20px)",borderTop:`1px solid ${G.border}`,padding:"8px 0 env(safe-area-inset-bottom,10px)",display:"flex",justifyContent:"space-around",alignItems:"center"}}>
...
export default function Mesajlar(){
  ...
  function yaziyorFn(){
    ...
    setYaziyorTimer(setTimeout(()=>{},2000));
  }
  ...
  if(!user)return(
    <div style={{minHeight:"100vh",background:G.black,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,fontFamily:G.fontBody}}>
      ...
      <a href="/" style={{padding:"12px 28px",borderRadius:14,background:G.goldGrad,color:"#0d0d0d",fontWeight:800,fontSize:13,textDecoration:"none",letterSpacing:"0.06em",textTransform:"uppercase"}}>Giriş Yap</a>
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:G.black,color:G.text,fontFamily:G.fontBody,display:"flex",flexDirection:"column"}}>
      ...
      {/* TOPBAR */}
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(8,8,8,0.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${G.border}`,padding:"10px 16px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        {aktif?(
          <>
            <button onClick={()=>{setAktif(null);setMesajlar([]);}} style={{background:"none",border:"none",padding:4,cursor:"pointer",color:G.textMuted,display:"flex"}}>
            ...
            <button onClick={()=>setNotAcik(p=>!p)} style={{width:34,height:34,borderRadius:10,background:notMetin?`rgba(212,175,55,0.12)`:"rgba(245,240,232,0.04)",border:`1px solid ${notMetin?G.gold+"40":G.border}`,color:notMetin?G.gold:G.textMuted,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
              <Icon id="pin" size={14} color={notMetin?G.gold:G.textMuted}/>
            </button>
          </>
        ):(
          <>
            ...
            <button onClick={()=>setYeniSohbet(true)} style={{width:34,height:34,borderRadius:10,background:G.goldGrad,border:"none",color:"#0d0d0d",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
              <Icon id="pencil" size={14} color="#0d0d0d" strokeWidth={2.5}/>
            </button>
          </>
        )}
      </div>
      ...
      {yeniSohbet&&(
        <>
          <div onClick={()=>{setYeniSohbet(false);setAramaKullanici("");setKullaniciSonuc([]);}} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(8px)"}}/>
          <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:201,background:"#0d0d0d",borderTop:`1px solid ${G.border}`,borderRadius:"24px 24px 0 0",padding:"20px 20px env(safe-area-inset-bottom,20px)",maxHeight:"80vh",display:"flex",flexDirection:"column",boxShadow:`0 -20px 60px rgba(0,0,0,0.8)`}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexShrink:0}}>
              ...
              <button onClick={()=>{setYeniSohbet(false);setAramaKullanici("");setKullaniciSonuc([]);}} style={{background:"rgba(245,240,232,0.05)",border:`1px solid ${G.border}`,borderRadius:8,padding:"5px 10px",color:G.textMuted,cursor:"pointer"}}>ESC</button>
            </div>
            <div style={{position:"relative",marginBottom:14,flexShrink:0}}>
              ...
              <input value={aramaKullanici} onChange={e=>kullaniciAra(e.target.value)} placeholder="Kullanıcı ara..." autoFocus style={{width:"100%",background:"rgba(245,240,232,0.05)",border:`1px solid ${G.border}`,borderRadius:12,padding:"9px 12px 9px 34px",color:G.text,fontSize:13,outline:"none",fontFamily:G.fontBody}}/>
            </div>
            <div style={{overflowY:"auto",flex:1}}>
              ...
              {kullaniciSonuc.map(u=>(
                <button key={u.id} onClick={()=>{sohbetAc({id:u.id,diger:u});setYeniSohbet(false);setAramaKullanici("");setKullaniciSonuc([]);}} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"12px",borderRadius:14,background:"none",border:`1px solid transparent`,cursor:"pointer",textAlign:"left",marginBottom:4,transition:"all 0.2s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=G.border;e.currentTarget.style.background="rgba(245,240,232,0.03)";}}
      ...
      {/* KONUŞMALAR LİSTESİ */}
      {!aktif&&(
        <div style={{flex:1,maxWidth:680,width:"100%",margin:"0 auto",paddingBottom:90,width:"100%"}}>
          {konusmalar.length===0?(
            <div style={{textAlign:"center",padding:"80px 20px"}}>
              <div style={{fontFamily:G.fontDisp,fontSize:52,color:G.textDim,letterSpacing:"0.1em",marginBottom:12}}>MESAJ YOK</div>
              <p style={{fontSize:14,color:G.textMuted,marginBottom:24}}>İlk mesajı sen gönder.</p>
              <button onClick={()=>setYeniSohbet(true)} style={{padding:"12px 28px",borderRadius:14,background:G.goldGrad,border:"none",color:"#0d0d0d",fontSize:13,fontWeight:800,cursor:"pointer",letterSpacing:"0.06em",textTransform:"uppercase"}}>
                ✏️ Yeni Mesaj Başlat
              </button>
            </div>
          ):konusmalar.map((k,i)=>{
            ...
            return(
              <button key={k.id} onClick={()=>sohbetAc(k)} style={{width:"100%",display:"flex",alignItems:"center",gap:13,padding:"14px 20px",borderBottom:`1px solid ${G.border}`,background:k.okunmayan>0?`${G.gold}05`:"transparent",border:"none",cursor:"pointer",textAlign:"left",transition:"background 0.15s"}}
                onMouseEnter={e=>e.currentTarget.style.background="rgba(245,240,232,0.03)"}
                onMouseLeave={e=>e.currentTarget.style.background=k.okunmayan>0?`${G.gold}05`:"transparent"}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    {k.okunmayan>0&&<span style={{minWidth:20,height:20,borderRadius:10,background:G.red,color:"#fff",fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 5px",flexShrink:0}}>{k.okunmayan}</span>}
                  </div>
                </div>
              </button>
            );
      ...
      {aktif&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",maxWidth:680,width:"100%",margin:"0 auto"}}>
          ...
          {/* SABİT NOT ALANI */}
          {notAcik&&(
            <div style={{background:`rgba(212,175,55,0.06)`,borderBottom:`1px solid ${G.gold}25`,padding:"12px 16px",flexShrink:0}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <p style={{fontSize:11,fontWeight:800,color:G.gold,textTransform:"uppercase",letterSpacing:"0.08em"}}>📌 Özel Not — sadece sen görürsün</p>
                <button onClick={()=>setNotAcik(false)} style={{background:"none",border:"none",color:G.textMuted,cursor:"pointer",display:"flex"}}><Icon id="x" size={14} color={G.textMuted}/></button>
              </div>
              <textarea value={notMetin} onChange={e=>setNotMetin(e.target.value)} placeholder="Bu sohbet hakkında not al..." rows={3} style={{width:"100%",background:"transparent",border:"none",color:G.text,fontSize:13,outline:"none",resize:"none",fontFamily:G.fontBody,lineHeight:1.6,marginBottom:8}}/>
              <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
                <button onClick={()=>{try{localStorage.removeItem("sf_not_"+aktif.id);}catch(e){}setNotMetin("");setNotKaydedildi(false);}} style={{padding:"5px 12px",borderRadius:8,background:"none",border:`1px solid ${G.gold}30`,color:G.gold,fontSize:11,cursor:"pointer"}}>Temizle</button>
                <button onClick={()=>{try{localStorage.setItem("sf_not_"+aktif.id,notMetin);}catch(e){}setNotKaydedildi(true);setTimeout(()=>setNotKaydedildi(false),1500);setNotAcik(false);}} style={{padding:"5px 14px",borderRadius:8,background:G.goldGrad,border:"none",color:"#0d0d0d",fontSize:11,fontWeight:800,cursor:"pointer"}}>
          ...
          {!notAcik&&notMetin&&(
            <div onClick={()=>setNotAcik(true)} style={{background:`rgba(212,175,55,0.04)`,borderBottom:`1px solid ${G.gold}18`,padding:"8px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
              <Icon id="pin" size={11} color={G.gold}/>
              <p style={{fontSize:12,color:G.gold,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>{notMetin}</p>
              ...
              return(
                <div key={m.id||i} style={{display:"flex",flexDirection:benim?"row-reverse":"row",gap:8,marginBottom:oncekiBenim?4:12,animation:"fadeUp 0.2s ease"}}>
                  ...
                  <div style={{maxWidth:"72%"}}>
                    {m.ses_url?(
                      <div style={{background:benim?`linear-gradient(135deg,${G.goldD},${G.gold})`:"rgba(245,240,232,0.06)",border:`1px solid ${benim?G.gold+"40":G.border}`,borderRadius:benim?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:"10px 14px"}}>
                      ...
                      <div style={{background:benim?`linear-gradient(135deg,${G.goldD},${G.gold})`:"rgba(245,240,232,0.06)",border:`1px solid ${benim?G.gold+"30":G.border}`,borderRadius:benim?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:"10px 14px"}}>
                        <p style={{fontSize:14,color:benim?"#0d0d0d":G.text,lineHeight:1.5}}>{m.metin}</p>
                      </div>
                    )}
                    <div style={{display:"flex",justifyContent:benim?"flex-end":"flex-start",gap:4,marginTop:3}}>
                      ...
                      {benim&&<p style={{fontSize:10,color:m.okundu?G.gold:G.textDim}}>{m.okundu?"✓✓":"✓"}</p>}
                    </div>
                  </div>
                </div>
              );
          ...
          {/* MESAJ YAZMA */}
          <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:"rgba(8,8,8,0.97)",backdropFilter:"blur(20px)",borderTop:`1px solid ${G.border}`,padding:"10px 12px env(safe-area-inset-bottom,10px)"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:1,background:`linear-gradient(90deg,transparent,${G.gold}18,transparent)`}}/>
            <div style={{display:"flex",gap:8,alignItems:"center",maxWidth:680,margin:"0 auto"}}>
              <button onClick={()=>dosyaRef.current?.click()} style={{width:36,height:36,borderRadius:"50%",background:"rgba(245,240,232,0.04)",border:`1px solid ${G.border}`,color:G.textMuted,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
                {gorselYukleniyor?<div style={{width:12,height:12,border:`2px solid ${G.border}`,borderTopColor:G.gold,borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>:<Icon id="image" size={15} color={G.textMuted}/>}
              </button>
              <input ref={dosyaRef} type="file" accept="image/*" onChange={gorselGonder} style={{display:"none"}}/>
              <input value={yeniMesaj} onChange={e=>{setYeniMesaj(e.target.value);yaziyorFn();}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();mesajGonder();}}} placeholder="Mesaj yaz..." style={{flex:1,background:"rgba(245,240,232,0.05)",border:`1px solid ${G.border}`,borderRadius:22,padding:"10px 16px",color:G.text,fontSize:13,outline:"none",fontFamily:G.fontBody}}/>
              {yeniMesaj.trim()?(
                <button onClick={()=>mesajGonder()} style={{width:36,height:36,borderRadius:"50%",background:G.goldGrad,border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
                  <Icon id="send" size={14} color="#0d0d0d" strokeWidth={2.5}/>
                </button>
              ):(
                <button onClick={sesKayd?sesDur:sesBaslat} style={{width:36,height:36,borderRadius:"50%",background:sesKayd?`linear-gradient(135deg,${G.red},${G.redL})`:"rgba(245,240,232,0.04)",border:`1px solid ${sesKayd?G.red:G.border}`,color:sesKayd?"#fff":G.textMuted,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,animation:sesKayd?"pulse 1s infinite":"none"}}>
