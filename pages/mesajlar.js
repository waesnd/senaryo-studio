...
function getC(dk) {
  return {
    ...
    muted: dk ? "rgba(241,245,249,0.45)" : "rgba(15,23,42,0.4)",
...
export default function Mesajlar() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,BlinkMacSystemFont,sans-serif", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", maxWidth: 680, width: "100%", margin: "0 auto" }}>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: dk ? "rgba(8,15,28,0.97)" : "rgba(255,255,255,0.97)", backdropFilter: "blur(28px)", borderTop: "1px solid " + C.border, padding: "10px 12px env(safe-area-inset-bottom,10px)" }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", maxWidth: 680, margin: "0 auto" }}>
              <button onClick={() => dosyaRef.current?.click()} style={{ width: 38, height: 38, borderRadius: "50%", background: C.input, border: "1px solid " + C.border, color: C.muted, fontSize: 17, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {gorselYukleniyor ? <div style={{ width: 14, height: 14, border: "2px solid " + C.border, borderTopColor: TEAL, borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /> : "🖼️"}
              </button>
              <input ref={dosyaRef} type="file" accept="image/*" onChange={gorselGonder} style={{ display: "none" }} />
              {/* Gönderi paylaş butonu */}
              <button onClick={() => {
                var url = window.location.origin;
                var msg = "📤 Bir göz at: " + url;
                setYeniMesaj(prev => prev + msg);
              }} style={{ width: 38, height: 38, borderRadius: "50%", background: C.input, border: "1px solid " + C.border, color: C.muted, fontSize: 17, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              {yeniMesaj.trim() ? (
                <button onClick={() => mesajGonder()} style={{ width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg," + TEAL + "," + TEAL_L + ")", border: "none", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <button onClick={sesKayd ? sesDur : sesBaslat} style={{ width: 38, height: 38, borderRadius: "50%", background: sesKayd ? "linear-gradient(135deg," + ACCENT + ",#c5180a)" : C.input, border: "1px solid " + (sesKayd ? ACCENT : C.border), color: sesKayd ? "#fff" : C.muted, fontSize: 17, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, animation: sesKayd ? "pulse 1s infinite" : "none" }}>
                  {sesKayd ? "⏹" : "🎤"}
                </button>
              )}
            </div>
      ...
      {!aktif && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 90, background: dk ? "rgba(8,15,28,0.97)" : "rgba(255,255,255,0.97)", backdropFilter: "blur(28px)", borderTop: "1px solid " + C.border, padding: "8px 0 env(safe-area-inset-bottom,8px)", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
          {[
